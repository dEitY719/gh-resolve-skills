...
classic Projects attached due to GraphQL deprecation (dEitY719/dotfiles#326 Bug B, same pattern
...
`gh api` 는 `--repo` 플래그를 받지 않으므로 repo 는 경로에 넣는다 (dEitY719/dotfiles#658).
...
지운다** (dEitY719/dotfiles#1403 / dEitY719/dotfiles#1407). `GH_HOST="$TARGET_HOST"` 가 그 서버를 고정한다.
